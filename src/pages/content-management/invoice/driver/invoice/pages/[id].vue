...
const { detailsInvoiceDriverDto } = storeToRefs(useInvoiceStore());
...
function deleteInv() {
  store.DeleteDriverInvoice([detailsInvoiceDriverDto.value.id]).then(() => router.go(-1))
}
        ...
        <VCol cols="12" md="10">
          <VCard>
            <VCardText class="flex flex-row">
              ...
              <VTextField v-model="detailsInvoiceDriverDto.number" readonly />
              <div class="flex mx-3 text-center items-center">
                <label>تاريخ الفاتورة</label>
              </div>
              <VTextField :value="new Date(detailsInvoiceDriverDto.dateCreated).toLocaleDateString()" readonly />
            </VCardText>
            <VDivider />
            <div v-if="detailsInvoiceDriverDto.orders.length > 0">
              <VTable class="mt-4">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="item in detailsInvoiceDriverDto.orders" :key="item.id">
            ...
            >
              <VCol cols="12" lg="8">
                <VTextarea
                  readonly
                  v-model="detailsInvoiceDriverDto.note"
                  label="الملاحظات"
                  class="mt-9"
                  rows="6"
                />
              ...
              <VCol cols="12" lg="4" class="OrderCol">
                <div class="flex items-center">
                  ...
                  <VTextField readonly>{{ detailsInvoiceDriverDto.orders.length }}</VTextField>
                </div>
                <div class="flex items-center">
                  <label class="w-40"> التكلفة</label>
                  <VTextField readonly>
                    {{
                      detailsInvoiceDriverDto.orders.reduce((pre, cur) => pre + cur.deliveryCost, 0)
                    }}
                  </VTextField>
                </div>
                <VDivider class="my-2" />
                ...
                <div class="flex items-center">
                  ...
                  <VTextField readonly>
                    {{
                      detailsInvoiceDriverDto.orders.reduce((pre, cur) => pre + cur.outComing, 0)
                    }}
                  </VTextField>
                </div>
                <div class="flex items-center">
                  <label class="w-40">المدفوع</label>
                  <VTextField readonly v-model="detailsInvoiceDriverDto.outComing"></VTextField>
                </div>
              </VCol>
            </VCardText>
          </VCard>
